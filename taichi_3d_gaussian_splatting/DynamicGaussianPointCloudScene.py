...
class DynamicGaussianPointCloudScene(torch.nn.Module):

    @dataclass
    class DynamicPointCloudSceneConfig(YAMLWizard):
    ...
    def __init__(
        ...
        config: DynamicPointCloudSceneConfig,
        ...
        self.point_cloud_t_base = nn.Parameter(point_cloud)

        self.config = config
        if point_cloud_features is not None:
            self.point_cloud_q_base = nn.Parameter(
                point_cloud_features[:, 0:4].clone())
            self.point_cloud_s = nn.Parameter(
                point_cloud_features[:, 4:7].clone())
            self.point_cloud_alpha = nn.Parameter(
                point_cloud_features[:, 7].clone())
            self.point_cloud_r_sh = nn.Parameter(
                point_cloud_features[:, 8:24].clone())
            self.point_cloud_g_sh = nn.Parameter(
                point_cloud_features[:, 24:40].clone())
            self.point_cloud_b_sh = nn.Parameter(
                point_cloud_features[:, 40:56].clone())
        else:
            buffer_size = self.point_cloud_t_base.shape[0]
            self.point_cloud_q_base = nn.Parameter(
                torch.zeros((buffer_size, 4)))
            self.point_cloud_s = nn.Parameter(torch.zeros((buffer_size, 3)))
            self.point_cloud_alpha = nn.Parameter(
                torch.zeros((buffer_size, 1)))
            self.point_cloud_r_sh = nn.Parameter(
                torch.zeros((buffer_size, 16)))
            self.point_cloud_g_sh = nn.Parameter(
                torch.zeros((buffer_size, 16)))
            self.point_cloud_b_sh = nn.Parameter(
                torch.zeros((buffer_size, 16)))
        ...
        self.register_buffer(
            "point_invalid_mask",
            torch.zeros(self.point_cloud_t_base.shape[0], dtype=torch.int8)
        )
        if point_object_id is None:
            point_object_id = torch.zeros(
                self.point_cloud_t_base.shape[0], dtype=torch.int32)
        self.register_buffer(
            "point_object_id",
            point_object_id
        )
    ...
    def forward(self):
        point_cloud_features = self._concat_to_point_cloud_features().contiguous()
        return self.point_cloud_t_base, point_cloud_features
    ...
    def initialize(self, point_cloud_rgb: Optional[torch.Tensor] = None):
        with torch.no_grad():
            ...
            """
            valid_point_cloud_np = self.point_cloud_t_base[self.point_invalid_mask == 0].detach(
            ...
            """
            self.point_cloud_s = torch.tensor(
                np.log(initial_covariance), dtype=torch.float32).unsqueeze(1)
            """
            self.point_cloud_s[(self.point_invalid_mask == 0), :] = torch.tensor(
                np.log(initial_covariance), dtype=torch.float32).unsqueeze(1)
            """
            ...
            self.point_cloud_q_base.copy_(torch.rand_like(
                self.point_cloud_q_base))
            self.point_cloud_q_base /= torch.norm(
                self.point_cloud_q_base, dim=-1, keepdim=True)

            # for alpha before sigmoid, we set it to 0.0, so sigmoid(alpha) is 0.5
            self.point_cloud_alpha.copy_(self.config.initial_alpha)
            # for color spherical harmonics factors, we set them to 0.5
            # self.point_cloud_features[:, 8] = 1.0
            self.point_cloud_r_sh[:, 0] = 1.0
            self.point_cloud_g_sh[:, 0] = 1.0
            self.point_cloud_b_sh[:, 0] = 1.0
            self.point_cloud_r_sh[:, 1:16] = 0.0
            self.point_cloud_g_sh[:, 1:16] = 0.0
            self.point_cloud_b_sh[:, 1:16] = 0.0
            if point_cloud_rgb is not None:
                point_cloud_rgb = torch.tensor(
                    point_cloud_rgb, dtype=torch.float32, requires_grad=False, device=self.point_cloud_t_base.device)
                point_cloud_rgb = point_cloud_rgb / 255.0
                point_cloud_rgb = torch.clamp(point_cloud_rgb, 0.0, 0.99)
                c0 = 0.28209479177387814
                self.point_cloud_r_sh[(self.point_invalid_mask == 0), 0] = \
                    self._logit(point_cloud_rgb[:, 0]) / c0
                self.point_cloud_g_sh[(self.point_invalid_mask == 0), 0] = \
                    self._logit(point_cloud_rgb[:, 1]) / c0
                self.point_cloud_b_sh[(self.point_invalid_mask == 0), 0] = \
                    self._logit(point_cloud_rgb[:, 2]) / c0

    def _logit(self, x: torch.Tensor) -> torch.Tensor:
        return torch.log(x / (1.0 - x))

    def _concat_to_point_cloud_features(self):
        return torch.cat([
            self.point_cloud_q_base,
            self.point_cloud_s,
            self.point_cloud_alpha,
            self.point_cloud_r_sh,
            self.point_cloud_g_sh,
            self.point_cloud_b_sh
        ], dim=1)

    def to_parquet(self, path: str):
        valid_point_cloud = self.point_cloud_t_base[self.point_invalid_mask == 0]
        point_cloud_features = self._concat_to_point_cloud_features()
        valid_point_cloud_features = point_cloud_features[self.point_invalid_mask == 0]
    ...
    @staticmethod
    def from_parquet(path: str, config=DynamicPointCloudSceneConfig()):
        ...
        if config.add_sphere:
            scene_df = DynamicGaussianPointCloudScene._add_sphere(
        ...
        if not set(feature_columns).issubset(set(scene_df.columns)):
            scene = DynamicGaussianPointCloudScene(
                point_cloud, config)
